...
import { FlexSearchBackend } from './flexsearch.js'
...
describe('FlexSearchBackend', () => {
  const backend = new FlexSearchBackend()
  ...
  it('returns results for keyword query', async () => {
    const results = await backend.query('driver licence')
    expect(results.length).toBeGreaterThan(0)
  })
